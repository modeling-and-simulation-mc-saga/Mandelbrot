...
public class Mandelbrot {
    ...
    protected Color onePoint(Point2D.Double c) {
        ...
        for (int i = 1; i <= maxIter; i++) {
            double x = p.x * p.x - p.y * p.y + c.x;
            double y = 2. * p.x * p.y + c.y;
    ...
    public void selectCenter(Point center) {
        double x = ((double) center.x / dimension.width) * range + origin.x - range / 2;
        double y = ((double) center.y / dimension.height) * range + origin.y - range / 2;
        origin.setLocation(x, y);
    }
